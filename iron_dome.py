...
def get_tech_data(ticker):
    ...
    try:
        ...
        if (hist is None or hist.empty) and ".TW" in symbol:
            stock = yf.Ticker(symbol.replace(".TW", ".TWO"))
            hist = stock.history(period="6mo")
...
with st.sidebar:
    ...
    st.divider()
    # 預設顯示您的持股
    default_my = "0052, 2330, 4958, 4420, 006208, 00981A"
    my_stocks = st.text_area("📋 目前持股監控 (請輸入代號)", default_my)
    
    default_monster = "3324, 3017, 1519, 3231, 2603, 2317"
    ...
    if not api_key: st.warning("請設定 API Key")
        ...
        for t in h_list:
            if d := get_tech_data(t):
                if d['vol_ratio'] > 1.3 and 55 < d['rsi'] < 75:
                    candidates.append(d)
        ...
        if candidates:
            ...
            for c in candidates:
                with st.expander(f"🔥 妖股報告：{c['name']} ({c['code']})", expanded=True):
                    prompt = f"分析台股妖股 {c['name']}。爆量且 RSI 轉強。參考新聞：{news[:5]}。請給出獵殺建議。100字內。"
                    res = client.models.generate_content(model='gemini-2.0-flash', contents=prompt)
                    st.write(res.text)
        else: st.warning("目前尚未發現符合條件的標的。")
